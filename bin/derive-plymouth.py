#!/usr/bin/env python3
"""Derive a Matrix boot splash from the Plymouth this machine has.
...
It installs as a SEPARATE theme, /usr/share/plymouth/themes/omarchy-matrix/, so
Omarchy's own is never overwritten: going back is `omarchy plymouth reset`.
...
THEME = "omarchy-matrix"
TARGET = Path("/usr/share/plymouth/themes") / THEME

# The four lines from the start of the film, in order.
LINES = [
    "Wake up, Neo...",
    "The Matrix has you...",
...
def matrix_block():
    ...
    return f'''
#----------------------------------------- Matrix -----------------------------
# Put here by omarchy-matrix. DO NOT edit this file by hand: it is derived again
...
Name=Omarchy Matrix
Description=Omarchy's splash with the line from the start of the film.
...
def main():
    ...
    theme_dir = Path(subprocess.run(["omarchy-theme-dir", "matrix"],
    ...
    staging = Path(tempfile.mkdtemp(prefix="matrix-plymouth."))
    try:
        ...
        if stage_only:
            out = Path.home() / ".cache/omarchy-matrix/plymouth"
