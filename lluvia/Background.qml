...
Item {
  ...
  // El fondo vivo no es un mecanismo aparte: es un fondo mas del carrusel. Se
  // envia backgrounds/0-lluvia-viva.png (un fotograma fijo que sirve de
  // miniatura y de reserva) y cuando ese es el seleccionado, en vez de pintar
  // la imagen se monta el shader de lluvia.
  readonly property string liveMarker: "0-lluvia-viva.png"
  readonly property bool liveRain: String(displayedBackground).indexOf(liveMarker) >= 0
  ...
  Variants {
    ...
    PanelWindow {
      ...
      readonly property int windowsHere: {
        try {
          ...
          for (var i = 0; i < ms.length; i++) {
            if (ms[i].name !== panel.modelData.name) continue
            var ws = ms[i].activeWorkspace
            if (!ws || !ws.lastIpcObject) return 0
      ...
      MatrixRain {
        ...
        running: root.liveRain && (!UPower.onBattery || panel.windowsHere === 0)
      }
      ...
      Image {
        ...
        anchors.fill: parent
        visible: !root.liveRain
        source: root.liveRain ? "" : root.imageUrl(root.displayedBackground)
      ...
      Image {
        ...
        smooth: true
        mipmap: true
        visible: !root.liveRain && root.oldBackground !== "" && root.revealProgress < 1
      ...
      Item {
        id: incomingLayer
        anchors.fill: parent
        visible: !root.liveRain && root.incomingBackground !== "" && incomingFrame.status === Image.Ready && (root.revealProgress >= 1 || panel.maskReady)
